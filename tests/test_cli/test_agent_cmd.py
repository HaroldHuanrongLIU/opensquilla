...
@pytest.mark.asyncio
async def test_run_agent_once_benchmark_profile_clamps_max_iterations_and_requires_artifact(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, Any] = {}
    ...
    contract = captured["tool_context"].run_contract
    assert contract.run_profile.value == "benchmark"
    assert contract.required_artifacts[0].extensions == (".pptx",)


@pytest.mark.asyncio
async def test_run_agent_once_applies_explicit_network_budget_overrides(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, Any] = {}
...
def test_top_level_agent_command_accepts_run_contract_options(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from opensquilla.cli.main import app
    ...
    assert result.exit_code == 0, result.output
    assert captured["run_profile"] == "benchmark"
    assert captured["required_artifacts"] == [".pptx"]
    assert captured["network_search_calls"] == 1
    assert captured["network_fetch_calls"] == 2
    assert captured["network_text_chars"] == 3456
    assert captured["network_per_fetch_chars"] == 789
